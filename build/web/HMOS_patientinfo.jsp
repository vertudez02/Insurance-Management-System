...
<body>
    ...
    <div id="page-container" class="fade page-sidebar-fixed page-header-fixed"><!-- begin #page-container -->	
        ...
        <div id="content" class="content">	<!-- begin of content #content -->
			...
			<!-- begin page-header -->
			<h1 class="page-header">List of Patient</h1>
			<!-- end page-header -->
			    ...
			    <div class="col-lg-12">
                    <div class="panel panel-inverse">
                       ...
                       <div class="panel-heading" style="background-color: rgb(0,100,157);">
                           <h1 class="panel-title" style="color:white;" >Patient Information</h1>
						</div>
                        <!-- end panel-heading -->
                        <!-- begin panel-body -->
                        <div class="panel-body">
                            ...
                            String data = "SELECT * FROM hms_patient_r_information ORDER BY pat_id DESC";
                            res = stat.executeQuery(data);
                            while(res.next()){
                               String status = res.getString("pat_status");
                            %> 
                            <tr>
                                <td hidden><%=res.getString("pat_id")%></td>
                                <td><%=res.getString("pat_firstname")+" "+res.getString("pat_middlename")+" "+res.getString("pat_lastname")%></td>
                                <td><%=res.getString("pat_age")%></td>
                                <td><%=res.getString("pat_birthdate")%></td>
                                <td><%=res.getString("pat_address")%></td>
                                <td><%=res.getString("pat_remarks")%></td>
                                <!--<td><span class="label label-primary"><%= res.getString("pat_status")%></span></td>-->
                                <td>
                               <%
                                if("Admitted".equals(status)){
                                out.println("<span class='label label-primary'>"+status+"</span>");
                                }
                                else if("Check-up".equals(status)){
                                out.println("<span class='label label-warning'>"+status+"</span>");
                                }
                                else if("Discharge".equals(status)){
                                out.println("<span class='label label-danger'>"+status+"</span>");
                                }
                               %>
                              </td>
                               <td>
                                          
                                <a href='HMOS_insuredaddnew.jsp?u=<%=res.getString("pat_id")%>' class="btn btn-primary"><i class='fa fa-eye'></i></a>
